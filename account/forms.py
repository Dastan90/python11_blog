...
class ChangePasswordForm(forms.Form):
    ...
    def clean(self):
        ...
        if new_pass != new_pass_confirm:
            raise forms.ValidationError('Неверное подтверждение нового паролья')
        return self.cleaned_data
...
class ForgotPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('Пользаватель не найден')
        return email
    ...
    def send_new_password(self):
        ...
        user.set_password(new_pass)
        user.save()
        send_mail('Восстановление пароля', f'Ваш новый пароль: {new_pass}', 'test2gmail.com', [email])
